...
example : interp (State.handler (M := ITree None)) p1 = interp State.handler p2 := by
  rw [p1, p2]
  -- 1. unroll loops
  conv =>
    lhs
  ...
  simp [interp_pure, interp_bind, interp_ite, Mem.update, State.handler, interp_trigger, Mem.write, Mem.read]
