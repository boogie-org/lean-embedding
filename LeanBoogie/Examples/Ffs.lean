...
set_option trace.LeanBoogie.normConTy true
...
open ITree EffProd ffs_imp
macro "mySimp" : tactic => `(tactic|
  simp only [↓nextBlock, ↓normTyA, ↓reduceIte,
    ConA.get, ConA.set,
    ...
    BitVec.ofNat_eq_ofNat, dite_eq_ite,
    ite_not, decide_eq_true_eq,

    -- bv.ne, bv.and, bv.add, bv.slt, bv.sext_8_32, bv.trunc_32_8, bv.lshr, bv.shl, BitVec.toNat
  ]
)
...
-- theorem th {h : i0 < 1} : ffs_imp (i0, ()) = ffs_ref (i0, ()) := by
theorem th : ffs_imp (0, ()) = ffs_ref (0, ()) := by
  unfold ffs_imp ffs_ref
  dsimp [default, ConA.inhabited, TyA.inhabited]
  save
  mySimp; first | rw [State.exe_ite]; maybeBranch | trySMT
  mySimp; first | rw [State.exe_ite]; maybeBranch | trySMT
  mySimp; first | rw [State.exe_ite]; maybeBranch | trySMT
  mySimp; first | rw [State.exe_ite]; maybeBranch | trySMT
  mySimp; first | rw [State.exe_ite]; maybeBranch | trySMT
